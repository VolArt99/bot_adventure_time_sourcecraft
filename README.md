...
- `/add_participant_manual <event_id> <user_id|@username> [going|waitlist]` — ручное добавление участника.
- `/send_event_card <event_id>` — обновить/переотправить карточку мероприятия (организатор/ответственный/админ).
