...
def build_member_help_text() -> str:
    return (
        ...
        "• /split_bill_status <id> — текущий статус, участники и оплаты.\n"
        "• /split_bill_paid <id> — отметить свою оплату.\n"
        "• /split_bill_join <id> — присоединиться по id (альтернатива кнопке).\n"
        "• /split_bill_close <id> — закрыть чек организатором после всех оплат.\n"
    )
