...
async def process_title(message: Message, state: FSMContext):
    ...
    await message.answer("📄 Введите описание (или 'пропустить'):", reply_markup=cancel_keyboard())
...
async def process_datetime(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("⏱ Введите длительность в часах (или 'пропустить'):\nПример: 2.5", 
                            reply_markup=cancel_keyboard())
    except ValueError:
...
async def process_price(message: Message, state: FSMContext):
    ...
    await message.answer("👥 Введите лимит участников (число или 'без лимита'):", 
                        reply_markup=cancel_keyboard())
...
async def process_limit(message: Message, state: FSMContext):
    ...
    await message.answer("🚗 Нужен карпулинг? (да/нет):", reply_markup=cancel_keyboard())
...
async def process_carpool(message: Message, state: FSMContext):
    ...
    try:
        ...
        if topics:
            ...
            await message.answer("📁 Выберите тему для публикации:", 
    ...
    await message.answer("🏷 Введите категорию (спорт, прогулки, поездки, игры, культура, еда, обучение):", 
                        reply_markup=cancel_keyboard())
